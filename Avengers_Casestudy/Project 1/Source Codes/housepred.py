...
class houseForm(FlaskForm):
    Lot_Area = TextField('Lot Area (SF)')
    Year_Built = TextField('Year Built')
    Year_RemodAdd = TextField('Year Remodel/Add')
    Mas_Vnr_Area = TextField('Masonry veneer Area (SF)')
    BsmtFin_SF_1 = TextField('Basement type 1 Area (SF)')
    Total_Bsmt_SF = TextField('Basement total Area (SF)')
    Second_flr_SF = TextField('Second Floor Area (SF)')
    Gr_Liv_Area = TextField('Above Ground Living Area (SF)')
    garage_yr_blt = TextField('Garage Year Built')
    Garage_Area = TextField('Garage Area (SF)')
    Wood_Deck_SF = TextField('Wood Deck (SF)')
    Open_Porch_SF = TextField('Open Porch (SF)')
    screen_porch = TextField('Screen Porch (SF)')
